...
type FilterKey = 'riskLevel' | 'branch' | 'city' | 'provider' | 'dateFrom' | 'dateTo';
...
export class ClaimsFiltersComponent implements OnInit, OnDestroy {
  ...
  get activeFilters(): { key: FilterKey; label: string }[] {
    ...
    if (v.city)      result.push({ key: 'city',      label: v.city });
    if (v.provider)  result.push({ key: 'provider',  label: v.provider });
  ...
  private riskLabel(level: string): string {
    const map: Record<string, string> = {
      verde: 'Bajo riesgo', amarillo: 'Medio riesgo', rojo: 'Alto riesgo', critico: 'Crítico',
    };
