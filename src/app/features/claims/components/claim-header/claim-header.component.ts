...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, AppButtonComponent, FlowStatusBadgeComponent, RiskBadgeComponent],
  template: `
    <header class="detail-header">
      ...
      <div class="detail-header__actions">
        ...
        <app-flow-status-badge [status]="claim.flowStatus"></app-flow-status-badge>
        <app-button label="Evaluar nuevamente" [loading]="evaluating" (pressed)="evaluate.emit()"></app-button>
        <app-button label="Explicar con IA" variant="secondary" [loading]="askingAgent" (pressed)="askAgent.emit()"></app-button>
      </div>
    </header>
  `,
})
